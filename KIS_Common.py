...
# 설정 파일 정보를 읽어 옵니다.
with open('/home/agh/stock_bot/myStockInfo.yaml', encoding='UTF-8') as f:
...
def GetOhlcv1(area, stock_code, limit=500, adj_ok="1"):
    ...
    df[['open', 'high', 'low', 'close', 'volume', 'change']] = df[
        ['open', 'high', 'low', 'close', 'volume', 'change']].apply(pd.to_numeric)
    ...
    if area == "KR":
        ...
        if do_amt > 0:
            ...
            try:
                ...
                AutoLimitData['OrderTime'] = GetNowDateStr(area) + str(time_info.tm_hour) + str(time_info.tm_min) + str(
                    time_info.tm_sec)
        ...
        AutoLimitData['DelDate'] = GetFromNowDateStr(area, "NONE",
                                                     10)  # 10일 미래의 날짜 즉 10일 후에는 해당 데이터를 삭제처리할 예정 (어자피 하루만 유효한 주문들이다. 하루 지나고 삭제해도 되지만 참고를 위해 10일간 남겨둔다)
        ...
        AutoLimitData['Id'] = AutoLimitData['NowDist'] + botname + area + str(stock_code) + str(
            AutoLimitData["OrderNum"]) + str(AutoLimitData["OrderNum2"]) + str(AutoLimitData["OrderTime"]) + str(
            do_amt) + str(target_price)
...
def SaveAutoLimitDoAgainData(AutoLimitData):
    # 파일 경로입니다.
    auto_order_file_path = "/var/autobot/" + AutoLimitData['Area'] + "_" + AutoLimitData['NowDist'] + "_" + \
                           AutoLimitData['BotName'] + "AutoOrderList.json"
                ...
                if AutoLimitData['Id'] == AutoOrderId:
                    ...
                    try:
                        ...
                        for OrderInfo in OrderList:
                            if OrderInfo['OrderNum'] == AutoLimitData['OrderNum'] and float(
                                    OrderInfo["OrderNum2"]) == float(AutoLimitData['OrderNum2']):
                                GapAmt = abs(OrderInfo["OrderResultAmt"] - OrderInfo["OrderAmt"])

                                if AutoLimitData['Area'] == "KR":
                                    KisKR.CancelModifyOrder(AutoLimitData['StockCode'], AutoLimitData['OrderNum'],
                                                            AutoLimitData['OrderNum2'], abs(GapAmt),
                                                            KisKR.GetCurrentPrice(AutoLimitData['StockCode']), "CANCEL")
                                else:
                                    KisUS.CancelModifyOrder(AutoLimitData['StockCode'], AutoLimitData['OrderNum2'],
                                                            AutoLimitData['OrderAmt'],
                                                            KisUS.GetCurrentPrice(AutoLimitData['StockCode']), "CANCEL")
                    ...
                    if Is_Except == True:
                        try:
                            ...
                            if AutoLimitData['Area'] == "KR":
                                ...
                                KisUS.CancelModifyOrder(AutoLimitData['StockCode'], AutoLimitData['OrderNum2'],
                                                        abs(GapAmt), KisUS.GetCurrentPrice(AutoLimitData['StockCode']),
                                                        "CANCEL")
                        except Exception as e:
                            print("Exception", e)
