...
def MarketStatus(stock_code='069500'):
    ...
    if res.status_code == 200 and res.json()["rt_cd"] == '0':
        ...
        result = {
            'Status': output1['new_mkop_cls_code'][0],
            # '','1' : 장개시전,  '2' : 장중,  '3' : 장종료후,  '4' : 시간외단일가,  '0' : 동시호가(개장전,개장후)
        }
    ...
    if int(Common.GetPrdtNo(Common.GetNowDist())) == 29:
        ...
        if res.status_code == 200 and res.json()["rt_cd"] == '0':
            ...
            balanceDict['RemainMoney'] = float(balanceDict['TotalMoney']) - float(
                balanceDict['StockMoney'])  # result['dnca_tot_amt']
...
def GetBalanceIRP():
    ...
    if res.status_code == 200 and res.json()["rt_cd"] == '0':
        ...
        balanceDict['RemainMoney'] = float(
            Data['RemainMoney'])  # float(balanceDict['TotalMoney']) - float(balanceDict['StockMoney'])
...
def CancelModifyOrder(stockcode, order_num1, order_num2, order_amt, order_price, mode="CANCEL", order_type="LIMIT",
    ...
    # 퇴직연금(29) 반영
    if int(Common.GetPrdtNo(Common.GetNowDist())) == 29:
        return CancelModifyOrderIRP(stockcode, order_num1, order_num2, order_amt, order_price, mode, order_type,
                                    order_dist)
    else:
...
def CancelModifyOrderIRP(stockcode, order_num1, order_num2, order_amt, order_price, mode="CANCEL", order_type="LIMIT",
                         order_dist="NONE"):
    time.sleep(0.2)
    # 모의계좌는 초당 2건만 허용하게 변경 - 24.04.01
    if Common.GetNowDist() == "VIRTUAL":
...
def CancelAllOrders(stockcode="", side="ALL"):
    ...
    for order in OrderList:
        if order['OrderSatus'].upper() == "OPEN":
            pprint.pprint(
                CancelModifyOrder(order['OrderStock'], order['OrderNum'], order['OrderNum2'], order['OrderAmt'],
                                  order['OrderAvgPrice']))
...
def GetMarketOrderPrice(stockcode, ResultOrder):
    ...
    for orderInfo in OrderList:
        if orderInfo['OrderNum'] == ResultOrder['OrderNum'] and float(orderInfo['OrderNum2']) == float(
                ResultOrder['OrderNum2']):
...
def GetETF_Nav(stock_code, Log="N"):
    ...
    if IsExcept == True:
        try:
            ...
            df = stock.get_etf_price_deviation(Common.GetFromNowDateStr("KR", "NONE", -5), Common.GetNowDateStr("KR"),
                                               stock_code)
...
def GetETFGapAvg(stock_code, Log="N"):
    ...
    try:
        df = stock.get_etf_price_deviation(Common.GetFromNowDateStr("KR", "NONE", -120), Common.GetNowDateStr("KR"),
                                           stock_code)
        if Log == 'Y':
            pprint.pprint(df)
        if len(df) == 0:
